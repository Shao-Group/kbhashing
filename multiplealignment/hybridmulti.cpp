...
double tanc1 = 0, tanc2 = 0;

vector<seed> get_bmers(readinfo* r, string s, int k, int b)
{	
	vector<seed> ret;
	int st,en;
    ...
    for(auto b: r->minbnlist)
    {
		...
		//cout<<tmp.hashval<<" "<<tmp.st<<" "<<tmp.en<<endl;
		ret.push_back(tmp);
	}
	tanc1 += ret.size();
	...
	for(int st = 0; st + k <= len; st++)
		if(r->idlist[st] == -1)
		{
			...
			if(ret.size() > 0 && f[k][b] + vlen == ret.back().hashval && ret.back().en == g[k][b] + st)
				continue;

			seed tmp;
			tmp.hashval = f[k][b] + vlen;
			int x = k;
			int y = b;
			...
			reverse(tmp.index.begin(), tmp.index.end());
			tmp.st = tmp.index.front();
			tmp.en = tmp.index.back();
...
void subseq_match(int i1, int i2, int k, int blen, vector<int> align)
{
	int trueset = 0;
...
int main(int argc, const char * argv[])
{
    ...
    vlen = atoi(argv[3]);
    int mlen = atoi(argv[4]);

    int matchtype = atoi(argv[5]);
    kbhash* kb = new kbhash(klen, klen - blen, vlen, mlen);
	...
	while(cin>>t)
	{
	    ...
	    {
	        case 0:
	            seeds.push_back(get_bmers(kb->get_Bneighbor_subseq(t), t, klen, blen));
	            break;

	        case 2:
	            seeds.push_back(get_bmers(kb->get_Bneighbor_substr4gap(t), t, klen, blen));
	            break;

	        case 1:
	            seeds.push_back(get_bmers(kb->get_Bneighbor_substr2gap(t), t, klen, blen));
	            break;

	        case 3:
	            seeds.push_back(get_bmers(kb->get_Bneighbor(t), t, klen, blen));
	            break;
	    }
	...
	for(int i = 0; i < n; i++)
	{
		...
		for(int j = i+1; j < n; j++)
		{
			...
			subseq_match(j, i, klen, blen, align);
		}
	}
	...
	int m = n * (n-1) / 2;
	printf("%d, %d, %d, %d, %.2lf, %.2lf, %.2lf, %.2lf, %.2lf, %.2lf, %.2lf, %.2lf, %.2lf, %.2lf, %.2lf, %.2lf\n", klen, blen, vlen, mlen, totalanc / m, totalacc / m, tp1 / m, rc1 / m, bp1 / m, tp2 / m, rc2 / m, bp2 / m, totalnum / m, 1 - totalnum / m, totalil / m, abp / m);
